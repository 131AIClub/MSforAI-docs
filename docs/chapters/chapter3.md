...
$$
\hat{\mathbf{y}} = \mathbf{wX^T}
$$

我们使用均方损失, 尝试找到最优参数$\mathbf{w}^*$:

$$
loss = (\mathbf{\hat{y}} - \mathbf{y})(\mathbf{\hat{y}} - \mathbf{y})^\mathbf{T} = (\mathbf{wX^T-y})(\mathbf{wX^T-y})^\mathbf{T}
$$
...
$$
\begin{aligned}
\mathbf{d}loss &= \mathbf{d}[(\mathbf{wX^T-y})(\mathbf{wX^T-y})^\mathbf{T}] \\
&= 2(\mathbf{wX^T-y})\mathbf{d}(\mathbf{wX^T-y})^\mathbf{T} \\
&= 2(\mathbf{wX^T-y})[\mathbf{d(Xw^T)}-\mathbf{dy^T}] \\
&= 2(\mathbf{wX^T-y})[\mathbf{d(X)w^T} + \mathbf{Xdw^T-dy^T}]
\end{aligned}
$$
...
$$
2(\mathbf{wX^T-y})\mathbf{Xdw^T} 
$$

因此, $loss$对$\mathbf{w}$的梯度为:

$$
\nabla_\mathbf{w}loss = 2(\mathbf{wX^T-y})\mathbf{X}
$$
...
$$
\begin{aligned}
2(\mathbf{wX^T-y})\mathbf{X} &= \mathbf{0} \\
\mathbf{wX^TX-yX} &= \mathbf{0} \\
\mathbf{wX^TX} &= \mathbf{yX} \\
\mathbf{w} &= \mathbf{yX}(\mathbf{X^TX})^{-1}
\end{aligned}
$$
